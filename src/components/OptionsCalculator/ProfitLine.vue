...
const points = computed(() => {
  ...
  const startPrice = priceScaleData[0].price
  const startX = priceScaleData[0].x
  ...
  thePoints.push({
    ...
    type: 'start',
    profit: profit,
    y: centerY - profit * profitCurrentScale
  })

  const endPrice = priceScaleData[priceScaleData.length - 1].price
  const endX = priceScaleData[priceScaleData.length - 1].x
  ...
  for (let i = 0; i < checkedOptionsData.length; i++) {
    if (checkedOptionsData[i].checked) {
      const price = checkedOptionsData[i].strike_price
      const currentX = startX + (price - startPrice) * priceCurrentScale
...
const pathDataTop = computed(() => {
  ...
  pathData += ` L ${thePoints[thePoints.length - 1].x} ${height / 2 + 10} L ${thePoints[0].x} ${height / 2 + 10}`
  return pathData
})
...
const calculateProfit = (price) => {
  let profit = 0
  for (const option of checkedOptionsData) {
    if (option.checked) {
      if (option.type === 'Call') {
